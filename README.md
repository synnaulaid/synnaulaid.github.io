# synnaulaid.github.io
Personal Page

https://synnaulaid.github.io/

This project is licensed under the MIT License.

You are free to use, modify, and distribute this code. However, you MUST include the original copyright notice and the MIT License in all copies or substantial portions of the Software.

Please do not remove or replace the original author name in the license.

Giving credit (such as mentioning or linking back to this repository) is highly appreciated, but not required.

If you're not familiar with open source licenses, it's best not to modify the license text.

For more information about the MIT License, please visit: https://opensource.org/licenses/MIT

# Tech Stack
- Jekyll https://jekyllrb.com/
- GitHub Pages https://pages.github.com/
- Gem https://rubygems.org/
- Ruby https://www.ruby-lang.org/en/
- HTML https://developer.mozilla.org/en-US/docs/Web/HTML
- CSS https://developer.mozilla.org/en-US/docs/Web/CSS
- JavaScript https://developer.mozilla.org/en-US/docs/Web/JavaScript
- Markdown https://www.markdownguide.org/
- YAML https://yaml.org/

See the [introduction documentation](docs/intro.md) for more details on how to use this project as a template for your own personal page.

-- - -
# My Blog Posts
- [How to Create a Personal Page for free with Github Pages](https://synnaulaid.github.io/blog/20260407/how-to-create-a-personal-page-for-free)
- [Finally Hello World!](https://synnaulaid.github.io/blog/20260406/finally-hello-world/) 

[See All Posts](https://synnaulaid.github.io/blog/)